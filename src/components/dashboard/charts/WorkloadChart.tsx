...
export const WorkloadChart = ({ futureCounts }: { futureCounts: Record<string, number> }) => {
    const days = useMemo(() => {
        const result = [];
        for (let i = 0; i < 7; i++) {
                    ...
                    {days.map((day, i) => {
                        ...
                        return (
                            <div key={day.date} className="flex flex-col items-center gap-2 flex-1 group relative h-full justify-end">
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-wider ${isToday ? 'text-primary' : 'opacity-40'}`}>
                                    {day.label}
                                </span>
                            </div>
                        );
