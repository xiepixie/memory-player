    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button
                ...
                className={`btn btn-sm btn-ghost btn-square transition-all duration-150 hover:scale-105 hover:rotate-[15deg] active:scale-95 ${isOpen ? 'bg-primary/10 text-primary' : 'text-base-content/60'}`}
                title="Change Theme"
            >
                <Palette size={18} />
            </button>

            {/* PERFORMANCE: CSS transitions instead of AnimatePresence */}
            <div
                className={`absolute right-0 top-full mt-2 w-48 bg-base-100 rounded-xl shadow-xl border border-base-200 overflow-hidden z-50 dropdown-enter ${isOpen ? 'dropdown-open' : 'dropdown-closed'}`}
                aria-hidden={!isOpen}
            >
