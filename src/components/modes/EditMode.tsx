...
export const EditMode = () => {
    ...
    // OPTIMIZATION: Use useDeferredValue to keep editor responsive
    const deferredContent = useDeferredValue(content);
    const parsedPreview = useMemo(() => parseNote(deferredContent), [deferredContent]);
    ...
    const clearCurrentPreviewHighlight = useCallback(() => {
    ...
    // Optimized caret coordinates using a persistent mirror div
    ...
    const getCaretCoordinates = useCallback((element: HTMLTextAreaElement, position: number) => {
        ...
        // Clean up span only, keep div properties for next time (optimization: could cache styles if they don't change)
    ...
    const scrollToCloze = useCallback((id: number) => {
        ...
        textarea.scrollTo({
            top: Math.max(0, targetTop - clientHeight * 0.3),
            behavior: 'smooth'
        });

        // 2. Scroll Preview (Sync with the specific instance index)
        // Use a more specific selector if possible, or cache these
        ...
        if (targetEl) {
            ...
            targetEl.classList.remove('cloze-target-highlight');
            void targetEl.offsetWidth; // Force reflow
            targetEl.classList.add('cloze-target-highlight');
    ...
    const jumpToSiblingCloze = useCallback((direction: 'next' | 'prev') => {
        ...
        textarea.scrollTo({
            top: Math.max(0, targetTop - textarea.clientHeight * 0.3),
            behavior: 'smooth'
        });
    ...
    useEffect(() => {
        if (currentNote) {
            ...
            // deferredContent will update automatically
        }
    }, [currentNote]);
    ...
    // 避免 early return null 导致 hooks 数量不一致
    // 返回空占位元素而不是 null
            ...
            <div className="sticky top-0 z-30 flex items-center justify-between px-3 py-2 backdrop-blur-md bg-base-100/80 border-b border-base-200/50 transition-all gap-4">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div className={`flex items-center gap-1.5 text-xs font-medium ${isDirty ? 'text-warning' : 'text-base-content/50'}`} title={isDirty ? 'Unsaved Changes' : 'All Saved'}>
                        <motion.div
                            className={`w-2 h-2 rounded-full ${isDirty ? 'bg-warning shadow-[0_0_8px_rgba(250,189,0,0.5)]' : 'bg-base-content/20'}`}
                            animate={isDirty ? { scale: [1, 1.2, 1], opacity: [1, 0.8, 1] } : { scale: 1, opacity: 1 }}
                            transition={isDirty ? { duration: 2, repeat: Infinity, ease: "easeInOut" } : {}}
                        />
                    </div>
                ...
                <div className="flex items-center gap-1.5 absolute left-1/2 -translate-x-1/2">
                    ...
                    <div className="join bg-base-200/50 p-0.5 rounded-lg border border-base-content/5">
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="btn btn-xs btn-ghost join-item h-6 w-7 min-h-0 p-0 hover:bg-base-100" onClick={() => insertText('**', '**')} title="Bold (Ctrl+B)">
                            <Bold size={12} />
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="btn btn-xs btn-ghost join-item h-6 w-7 min-h-0 p-0 hover:bg-base-100" onClick={() => insertText('*', '*')} title="Italic (Ctrl+I)">
                            <Italic size={12} />
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="btn btn-xs btn-ghost join-item h-6 w-7 min-h-0 p-0 hover:bg-base-100" onClick={() => insertText('# ')} title="Heading 1">
                            <Heading1 size={12} />
                        </motion.button>
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="btn btn-xs btn-ghost join-item h-6 w-7 min-h-0 p-0 hover:bg-base-100" onClick={() => insertText('- ')} title="List">
                            <List size={12} />
                        </motion.button>
                    </div>
                            ...
                            `}
                                    ...
                                    >
                                        <span className="z-10 flex items-center gap-0.5">
                                            c{id}
                                            {isMulti && (
                                                <span className={`ml-0.5 px-1 rounded-full text-[8px] ${isTarget
