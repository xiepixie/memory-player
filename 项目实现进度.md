...
# 复习会话与评分系统实现进度（Review Session & Grading）
...
- **到期卡片拉取（fetchDueCards）**:
  - 通过 `dataService.getDueCards(limit)` 统一封装不同后端：
    - Supabase：
      - 从 `cards` 表筛选 `due <= now` 且 `is_suspended = false`，联结 `notes` 表过滤掉已被软删除的笔记.
      - 按 `due` 升序排序，限制返回条数 `limit`，并将 `note.relative_path + vault_id` 解析为绝对文件路径.
    - Mock：
