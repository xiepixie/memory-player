...
2.  **复习**：`Dashboard` 生成队列 -> [startSession](cci:1://file:///d:/memory-player/src/store/appStore.ts:704:2-721:3) -> [loadNote](cci:1://file:///d:/memory-player/src/store/appStore.ts:1015:2-1061:3)（内部通过 `loadContentFromSource` / `updateCacheAndIds` / `syncNoteMetadata` 完成加载与同步）-> 用户打分 -> [saveReview](cci:1://file:///d:/memory-player/src/lib/storage/types.ts:69:2-77:91) (FSRS calc) -> RPC 写库 -> 加载下一张.
