# 状态管理设计（Memory Player）
 ...
 - 以及辅助文档 docs/DEVELOPER_GUIDE_FSRS_FLOW.md
 ...
 - **数据服务抽象**：`DataService` 接口
   - 由 `MockAdapter`（本地开发/离线）与 `SupabaseAdapter`（云端）实现。
 ...
 **核心原则**：
 ...
 - `DataService` 负责「笔记 → 卡片 → FSRS 状态 → 历史」的持久化与同步。
 ...
 来源：`appStore` 中的 `createServiceSlice`。


### 3.1 状态字段

- `dataService: DataService`
  - 当前使用的后端实现（`MockAdapter` 或 `SupabaseAdapter`）。
 ...
 - `initDataService(type: 'mock' | 'supabase')`
   - 动态 `import` 对应的 Adapter 并调用 `init()`.
   - 若为 Supabase 模式，自动获取用户信息.
   - 成功后串行调用：`loadVaults()` → 确定 `currentVault` 后再调用 `loadAllMetadata()` → `loadReviewHistory()`.
 - `signOut()`
...
## 4. VaultSlice：Vault、文件与元数据
 ...
 来源：`appStore` 中的 `createVaultSlice`.
...
- `vaults` / `currentVault`：Vault 列表与当前选中项.
 ...
 - `handleExternalCardUpdate(row)` **[关键]**
...
## 5. SessionSlice：复习队列与打分
 ...
 来源：`appStore` 中的 `createSessionSlice`.
...
## 6. NoteSlice：当前笔记
 ...
 来源：`appStore` 中的 `createNoteSlice`.
    ...
    - Supabase 实现：
      ...
      - 当 `after` 不为空（增量模式）：
        ...
        - 包含以下几类变更：
          ...
          - 整篇笔记被软删（`softDeleteNote`） -> Note 标记 `notes.is_deleted = true`，同时该 Note 下所有 Card 标记 `is_deleted = true, updated_at = now()`.
...
1.  **启动**：`initDataService` -> `loadVaults` -> `loadAllMetadata`.
2.  **复习**：`Dashboard` 生成队列 -> `startSession` -> `loadNote`（内部通过 `loadContentFromSource` / `updateCacheAndIds` / `syncNoteMetadata` 完成加载与同步）-> 用户打分 -> `saveReview` (FSRS calc) -> RPC 写库 -> 加载下一张.
3.  **同步**：本地保存文件 -> `FileSystemWatcher` -> `syncNote` -> 计算 Hash -> Upsert `cards`.
4.  **多端**：端 A 复习 -> Supabase 更新 -> 端 B 收到 Realtime -> `handleExternalCardUpdate` -> 更新端 B 内存状态.
