...
- **[FIXED] Redundant Parsing**: `EditMode` now checks the `active` prop before running `parseNote` or updating `debouncedContent`. This prevents the hidden editor instance (kept alive by `NoteRenderer`) from consuming CPU for parsing during Review sessions.
...
- **问题 7：Zustand persist 同步写入（localStorage 阻塞）**
  ...
  - **修复行动**：
    - **[FIXED] Persistence Optimization**: Removed `fileMetadatas`, `currentNote`, and `currentMetadata` from the `partialize` whitelist in `appStore.ts`. These large/derived objects are no longer serialized to localStorage on every state change. They are instead re-populated via `loadAllMetadata` / `loadNote` on application startup/navigation.
